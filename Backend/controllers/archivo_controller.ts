...
const fileFilter = (req: any, file: any, cb: any) => {
    ...
    if (extname && mimetype) {
        ...
        cb(new Error('Error: Tipo de archivo no soportado. Solo se permiten archivos de audio y video (mp3, mp4, wav, ogg).'));
    }
};
...
export const subirArchivo = (req: Request, res: Response) => {
    upload(req, res, async (err: any) => {
        ...
        if (err) return res.status(500).json({ mensaje: 'Error al subir archivo físico al servidor.' });
        ...
        try {
            ...
            res.status(200).json({ mensaje: 'Archivo subido y metadatos registrados correctamente.' });

        } catch (error: any) {
            console.error('Error en base de datos al subir archivo:', error);
            res.status(500).json({ mensaje: 'Error al guardar información en la base de datos.', detalle: error.message });
        }
    });
};
